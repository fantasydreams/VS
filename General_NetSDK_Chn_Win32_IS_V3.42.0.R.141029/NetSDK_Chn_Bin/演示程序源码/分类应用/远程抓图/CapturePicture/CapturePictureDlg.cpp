...
BOOL CCapturePictureDlg::OnInitDialog()
{
	...
	SetDlgItemText(IDC_IPADDRESS_IP, "172.6.5.33");
...
void CCapturePictureDlg::UpdateSnapConfig(int nChn, int nSnapMode)
{
	...
	if (bframeRateOutofRange)
	{
		...
		{
			CString msg;
			msg.Format(ConvertString("%d frame pre second"), m_stuSnapCfg[nChn].struSnapEnc[nSnapMode].byFramesPerSec);
			AfxMessageBox(msg);
		}
		else
		{
			CString msg;
			msg.Format(ConvertString("%d second pre frame"), 256 - m_stuSnapCfg[nChn].struSnapEnc[nSnapMode].byFramesPerSec);
			AfxMessageBox(msg);
		}
	}
