...
BOOL CAlarmDeviceDlg::OnInitDialog()
{
	...
	//Set initial IP address 
	{
		...
		if (csIp == CString("ip"))
		{
			m_DvrIPAddr.SetWindowText("172.5.2.103");
		} 
		else
		{
...
void CAlarmDeviceDlg::ReceiveRealData(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer, DWORD dwBufSize)
{
...
{
	CDlgCfgAnalogAlarm dlg(this, m_LoginID);
	dlg.DoModal();
}
