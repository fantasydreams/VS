...
BOOL DlgCfgAlarmOut::SetAlarmOutConfig()
{
	...
	CFG_ALARMOUT_INFO stuInfo = {0};
	stuInfo.nChannelID = nSelChn;
	GetDlgItemText(IDC_ALARMOUT_EDT_NAME, stuInfo.szChnName, sizeof(stuInfo.szChnName));
	GetDlgItemText(IDC_ALARMOUT_EDT_OUTTYPE, stuInfo.szOutputType, sizeof(stuInfo.szOutputType));
	stuInfo.nOutputMode = nSelMode;
	
	char szJson[1024] = {0};
	BOOL bRet = CLIENT_PacketData(CFG_CMD_ALARMOUT, &stuInfo, sizeof(stuInfo), szJson, sizeof(szJson));
	if (bRet)
	{
		int nerror = 0;
		int nrestart = 0;
