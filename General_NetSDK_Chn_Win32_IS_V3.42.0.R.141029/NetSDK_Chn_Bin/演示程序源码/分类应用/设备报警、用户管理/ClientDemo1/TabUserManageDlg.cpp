...
void CTabUserManageDlg::RefreshLevelList()
{
	...
	if(hSelItem != NULL)
	{
		...
		{
			dwNum = m_usersInfo.userList[dwTagData].dwRightNum;
			dwList = m_usersInfo.userList[dwTagData].rights;
		}
		else	//Group 
		{
...
void CTabUserManageDlg::OnModifyUser() 
{
	...
	if(0 < m_hLoginID)
	{
		...
		if(hSelItem != NULL)
		{
			...
			{
				CModifyUserInfo dlg;
				dlg.InitModifyUserInfoDlg(&m_usersInfo,dwData,m_hLoginID);
				int nResponse = dlg.DoModal();
				if (nResponse == IDOK)
				{
...
void CTabUserManageDlg::OnModifyPsw() 
{
	...
	if(0 < m_hLoginID)
	{
		...
		if(hSelItem != NULL)
		{
			...
			{
				CModifyPwd dlg;
				dlg.InitModifyPwdInfoDlg(&m_usersInfo,dwData,m_hLoginID);
				int nResponse = dlg.DoModal();
				if (nResponse == IDOK)
				{
					//Get user information 
...
void CTabUserManageDlg::OnAddGroup() 
{
	...
	{
		CAddGroupInfo dlg;
		dlg.InitAddGroupDlg(&m_usersInfo,m_hLoginID);
		int nResponse = dlg.DoModal();
		if (nResponse == IDOK)
		{
			//Get user information 
...
void CTabUserManageDlg::OnModifyGroup() 
{
	...
	if(0 < m_hLoginID)
	{
		...
		if(hSelItem != NULL)
		{
			...
			{
				USER_GROUP_INFO_EX *groupInfo = (USER_GROUP_INFO_EX *)dwData;
				int nCount = m_usersInfo.dwGroupNum;
				for (int i = 0; i < nCount; i++)
				{
					if (groupInfo->dwID == m_usersInfo.groupList[i].dwID)
					{
						CModifyGroupInfo dlg;
						dlg.InitModifyGroupInfoDlg(&m_usersInfo, i, m_hLoginID);
						int nResponse = dlg.DoModal();
						if (nResponse == IDOK)
						{
							//To get user information 
