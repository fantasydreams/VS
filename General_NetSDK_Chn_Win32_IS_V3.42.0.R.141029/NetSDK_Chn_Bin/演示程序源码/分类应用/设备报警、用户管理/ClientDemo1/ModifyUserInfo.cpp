...
void CModifyUserInfo::OnOK() 
{
	...
	GetDlgItem(IDC_EDIT_NEWNAME)->GetWindowText(userInfo.name, DH_USER_NAME_LENGTH_EX+1);
	memcpy(userInfo.passWord, m_usersInfo->userList[m_dwUserID].passWord, DH_USER_PSW_LENGTH);
	GetDlgItem(IDC_EDIT_NEWMEMO)->GetWindowText(userInfo.memo, DH_MEMO_LENGTH);
	userInfo.dwFouctionMask = ((CButton*)GetDlgItem(IDC_CHECK_NEWREUSEABLE))->GetCheck();
...
BOOL CModifyUserInfo::OnInitDialog() 
{
	...
	{
		if(m_usersInfo->groupList[i].dwID == dwGroupID)
		{
			...
			{
				((CButton*)GetDlgItem(IDC_CHECK_REUSEABLE))->SetCheck(m_usersInfo->userList[m_dwUserID].dwFouctionMask);
				((CButton*)GetDlgItem(IDC_CHECK_NEWREUSEABLE))->SetCheck(m_usersInfo->userList[m_dwUserID].dwFouctionMask);
			}
			//Initialize right list 
			InitLevelTree(m_usersInfo->userList[m_dwUserID].dwRightNum,m_usersInfo->userList[m_dwUserID].rights);
			break;
		}
	}
...
void CModifyUserInfo::InitModifyUserInfoDlg(USER_MANAGE_INFO_EX *info, DWORD dwID, long hLoginID)
{
	m_usersInfo = info;
	m_dwUserID = dwID;
	m_hLoginID = hLoginID;
