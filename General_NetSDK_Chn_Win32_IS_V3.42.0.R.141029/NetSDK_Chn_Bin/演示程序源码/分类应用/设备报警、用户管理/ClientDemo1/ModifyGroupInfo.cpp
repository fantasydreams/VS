...
void CModifyGroupInfo::OnOK() 
{
	...
	groupInfo.dwID = m_usersInfo->groupList[m_dwGroupID].dwID;
	GetDlgItem(IDC_EDIT_NEWGROUPNAME)->GetWindowText(groupInfo.name, DH_USER_NAME_LENGTH_EX+1);
	GetDlgItem(IDC_EDIT_NEWMEMO)->GetWindowText(groupInfo.memo, DH_MEMO_LENGTH);
...
BOOL CModifyGroupInfo::OnInitDialog() 
{
	...
	memcpy(szBuf, m_usersInfo->groupList[m_dwGroupID].name, DH_USER_NAME_LENGTH_EX);
	...
	memcpy(szBuf, m_usersInfo->groupList[m_dwGroupID].memo, DH_MEMO_LENGTH);
	strBuf = szBuf;
	GetDlgItem(IDC_EDIT_MEMO)->SetWindowText(strBuf);
	GetDlgItem(IDC_EDIT_NEWMEMO)->SetWindowText(strBuf);
	
	InitLevelTree(m_usersInfo->groupList[m_dwGroupID].dwRightNum, m_usersInfo->groupList[m_dwGroupID].rights);
...
void CModifyGroupInfo::InitModifyGroupInfoDlg(USER_MANAGE_INFO_EX *info, DWORD dwGroupID, long hLoginID)
{
	m_usersInfo = info;
