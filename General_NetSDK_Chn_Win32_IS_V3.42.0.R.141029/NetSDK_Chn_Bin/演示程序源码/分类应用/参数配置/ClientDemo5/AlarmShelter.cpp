 ...
 */
void CAlarmShelter::InitDlgInfo(DHDEV_ALARM_SCHEDULE_EX *pAlarmInfo, BYTE nAlarmIn, BYTE nAlarmOut, int nChannel, vector<string>& vecAlarmName) 
{
	if(pAlarmInfo != NULL)
	{
		...
		m_pstShelterInfo = new CFG_SHELTER_INFO[CHANNEL_NUM_LIMIT];
		if(NULL == m_pstShelterInfo)
		{
			return;
		}
		memset(m_pstShelterInfo, 0, CHANNEL_NUM_LIMIT*sizeof(CFG_SHELTER_INFO));
...
//---p3
		...
		m_chnAlarmOut.SetChnState(m_pstShelterInfo[nNum].stuEventHandler.dwAlarmOutMask[0],m_nAlarmOut);	
...
void CAlarmShelter::OnBtnDriveRecord() 
{
	// TODO: Add your control notification handler code here
	int nIndex = m_ctlChannel.GetCurSel();
	if (CB_ERR == nIndex)
	...
	BOOL bRet = recordChnDlg.SetRecordMark(m_pstShelterInfo[nNum].stuEventHandler.dwRecordMask, m_nChannel);
	if (!bRet)
	{
		return;
	}
	...
	{
		memcpy(m_pstShelterInfo[nNum].stuEventHandler.dwRecordMask, 
			recordChnDlg.GetRecordMark(), MAX_CHANNEL_COUNT*sizeof(DWORD));
		for (int i = 0; i < MAX_CHANNEL_COUNT; i++)
        {
			if (0 != m_pstShelterInfo[nNum].stuEventHandler.dwRecordMask[i])
			{
				m_pstShelterInfo[nNum].stuEventHandler.abRecordMask = true;
				break;
			}
        }
	}
