...
void CClientDemo5Dlg::GetRemoteStorageLimit(CFG_REMOTE_STORAGELIMIT_GROUP *pstRemoteStorage, int nCurChannel)
{
	...
	int nerror = 0;
	BOOL bSuccess = CLIENT_GetNewDevConfig(m_LoginID, CFG_CMD_REMOTE_STORAGE_LIMIT, nCurChannel, szOutBuffer, 256*1024, &nerror, 3000);
	if(bSuccess)
	{
		int nRetLen = 0;
		BOOL bRet = CLIENT_ParseData(CFG_CMD_REMOTE_STORAGE_LIMIT, szOutBuffer, pstRemoteStorage, sizeof(CFG_REMOTE_STORAGELIMIT_GROUP),&nRetLen);
		if(!bRet)
		{
